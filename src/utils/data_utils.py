...
def get_length(dataset: datasets.Dataset) -> int:
    """
    Get the length of the training dataset, limiting to 1000 examples for training.
    Args:
        dataset (DatasetDict): The dataset containing a 'train' split.
    Returns:
        int: The length of the training dataset, capped at 1000.
    """
    length_all = len(dataset)

    length = min(length_all, 1000)
    logger.info(f"Dataset length: {length_all} (using {length} for training)")
    return length
...
def get_dataset(args: ScriptArguments) -> DatasetDict:
    ...
    if args.dataset_name and not args.dataset_mixture:
        ...
        length = get_length(dataset["train"])
        dataset['train'] = dataset["train"].select(range(length))
        return dataset
        ...
        if datasets_list:
            ...
            if args.dataset_mixture.test_split_size is not None:
                ...
                length = get_length(dataset_dict["train"])
                dataset_dict["train"] = dataset_dict["train"].select(range(length))
                logger.info(f"Using dataset mixture with {len(dataset_dict['train'])} examples for training")
                return dataset_dict
        else:
